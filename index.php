            ...
            if ($action === 'mute') {
                ...
                if (!($muteRes['ok'] ?? false)) {
                    ...
                    if (str_contains($muteErr, 'administrator')) {
                        sendReply($chatId, $replyTo ?? 0, '⛔ Нельзя замьютить администратора чата.');
                    } else {
    ...
    if ($count >= 3) {
        ...
        if (!($banRes['ok'] ?? false)) {
            ...
            if (str_contains($banErr, 'administrator')) {
                sendReply($chatId, $replyTo, '⛔ Нельзя заблокировать администратора чата.');
            } else {
                sendReply($chatId, $replyTo, "⛔ Не удалось выдать бан: {$banErr}");
            }
            return;
        }
