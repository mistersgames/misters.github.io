...
function cmdWho(int $chatId, int $replyTo, string $subject): void {
    $user1 = getRandomChatUser($chatId);
    ...
    // Пробуем получить второго участника (отличного от первого)
    $user2 = null;
    for ($i = 0; $i < 5; $i++) {
        $candidate = getRandomChatUser($chatId);
        if ($candidate && $candidate['user_id'] !== $user1['user_id']) {
            $user2 = $candidate;
            break;
        }
    }
    ...
    if ($user2 !== null) {
        $mister  = $makeLink($user1);
        $misters = $makeLink($user2);
