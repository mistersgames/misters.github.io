...
register_shutdown_function(function() {
    ...
    if ($e && in_array($e['type'], [E_ERROR, E_PARSE, E_CORE_ERROR, E_COMPILE_ERROR])) {
        file_put_contents($logFile, date('[Y-m-d H:i:s] ') . "[FATAL {$e['type']}] {$e['message']} line {$e['line']}\n", FILE_APPEND);
 ...
 * Чистый PHP, cURL, SQLite3, без зависимостей. Webhook.
 */
...
function initDB(): void {
    ...
    if ($amColRes) {
        while ($col = $amColRes->fetchArray(SQLITE3_ASSOC)) {
            $amCols[] = $col['name'];
        }
    }
    $amRequired = [
...
function registerBotChat(int $chatId, string $title): void {
    $db      = getDB();
    $escaped = $db->escapeString($title);
    $now     = time();
    $db->exec("
        INSERT INTO bot_chats (chat_id, title, joined_at) VALUES ($chatId, '$escaped', $now)
...
function sendOwnerPanel(int $chatId): void {
    ...
    $totalUsers = (int)$db->querySingle("SELECT COUNT(DISTINCT user_id) FROM users") ?: 0;
    $totalChats = (int)$db->querySingle("SELECT COUNT(*) FROM bot_chats") ?: 0;

    // Общее число наказаний: варны + муты + баны + темпбаны
    $totalWarns    = (int)$db->querySingle("SELECT COALESCE(SUM(count),0) FROM warns") ?: 0;
    $totalMutes    = (int)$db->querySingle("SELECT COUNT(*) FROM mutes") ?: 0;
    $totalBans     = (int)$db->querySingle("SELECT COUNT(*) FROM bans") ?: 0;
    $totalTempbans = (int)$db->querySingle("SELECT COUNT(*) FROM tempbans") ?: 0;
...
function saveUser(int $chatId, array $from, int $messageId = 0): void {
    $db       = getDB();
    $userId   = $from['id'];
    $username = strtolower($db->escapeString($from['username'] ?? ''));
    $name     = $db->escapeString(trim(($from['first_name'] ?? '') . ' ' . ($from['last_name'] ?? '')));
    $now      = time();

    $db->exec("
        INSERT INTO users (user_id, chat_id, username, name, message_count, joined_at)
        VALUES ($userId, $chatId, '$username', '$name', 1, $now)
...
if (!empty($msg['new_chat_members'])) {
    ...
    foreach ($msg['new_chat_members'] as $newMember) {
        ...
        $wRow = $db->querySingle("SELECT enabled, template FROM welcome_settings WHERE chat_id = $chatId", true);
        ...
        $now = time();
        $db->exec("
            INSERT INTO verification_pending (user_id, chat_id, message_id, joined_at)
        ...
        $uname   = strtolower($db->escapeString($username));
        $eName   = $db->escapeString($fullName);
        $db->exec("
            INSERT INTO users (user_id, chat_id, username, name, message_count, joined_at)
    ...
    if ($replyToId && $text !== '') {
        ...
        $pending = $db->querySingle(
            "SELECT * FROM pending_actions WHERE prompt_msg_id = $replyToId AND chat_id = $chatId",
            true
        );
        if ($pending && isChatAdmin($chatId, $userId)) {
            ...
            if ($action === 'mute') {
                $db->exec("
                    INSERT INTO mutes (user_id, chat_id, until) VALUES ($targetId, $chatId, $until)
                ...
                $reportedRow = $db->querySingle("SELECT name, username FROM users WHERE user_id = $targetId AND chat_id = $chatId", true);
                ...
                if ($result['ok'] ?? false) {
                    ...
                    if ($seconds > 0) {
                        $db->exec("
                            INSERT INTO tempbans (user_id, chat_id, until, notified) VALUES ($targetId, $chatId, $until, 0)
                            ON CONFLICT(user_id, chat_id) DO UPDATE SET until = $until, notified = 0
                        ");
                    }
                    $db->exec("
                        INSERT INTO bans (user_id, chat_id, banned_by, reason, banned_at)
                        VALUES ($targetId, $chatId, $userId, 'Репорт', $now)
                        ON CONFLICT(user_id, chat_id) DO UPDATE SET banned_by = $userId, banned_at = $now
                    ");
                    $db->exec("UPDATE reports SET reviewed = 1, reviewer_id = $userId WHERE id = $reportId");
                    $reportedRow = $db->querySingle("SELECT name FROM users WHERE user_id = $targetId", true);
                    $rName = htmlspecialchars($reportedRow['name'] ?? "ID {$targetId}", ENT_XML1);
                    $suffix = $seconds > 0 ? " на {$durationStr}" : ' навсегда';
                    ...
                    sendMessage($chatId, "❌ Не удалось заблокировать пользователя. Проверьте права бота.");
                }
            }
    ...
    if ($text && $text[0] !== '/') {
        ...
        if (preg_match('/^кто\s+(.+)/iu', $text, $m)) {
            cmdWho($chatId, $messageId, trim($m[1]));
            return;
        }

        // Триггер "Мистер кто [что-то]?" / "Мистерс кто [что-то]?"
        if (preg_match('/^мистерс?\s+кто\s+(.+)/iu', $text, $m)) {
            cmdWho($chatId, $messageId, trim($m[1]));
            return;
        }

        // Триггер "какова/какая вероятность [чего-то]?" или "вероятность [чего-то]?"
        if (preg_match('/^(?:(?:какова|какая)\s+)?вероятность(?:\s+что)?\s+(.+)/iu', $text, $m)) {
            cmdChance($chatId, $messageId, trim($m[1]));
            return;
        }

        // Триггер "Мистер вероятность [чего-то]?" / "Мистерс вероятность [чего-то]?"
        if (preg_match('/^мистерс?\s+вероятность(?:\s+что)?\s+(.+)/iu', $text, $m)) {
            cmdChance($chatId, $messageId, trim($m[1]));
            return;
        }
    ...
    if ($command === '/topstat') {
        $db    = getDB();
        $total = (int)$db->querySingle("SELECT COUNT(*) FROM users WHERE chat_id = $chatId");
        $withMsg = (int)$db->querySingle("SELECT COUNT(*) FROM users WHERE chat_id = $chatId AND message_count > 0");
        $maxMsg  = (int)$db->querySingle("SELECT MAX(message_count) FROM users WHERE chat_id = $chatId");
...
function handleCallback(array $cb): void {
    ...
    if (str_starts_with($data, 'verify:')) {
        ...
        $db      = getDB();
        $pending = $db->querySingle(
            "SELECT message_id FROM verification_pending WHERE user_id = $targetId AND chat_id = $chatId",
            true
        );
    ...
    if ($data === 'help_public') {
        ...
        $text = "👤 <b>Команды для всех участников</b>\n\n"
              ...
              . "   <code>!Репорт\nПричина жалобы</code>\n\n"
              . "<i>💡 Также можно написать слово «правила», «кто ...?», «вероятность ...?»</i>";
        apiRequest('editMessageText', [
            'chat_id'      => $chatId,
            'message_id'   => $messageId,
            'text'         => $text,
    ...
    if (preg_match('/^(unmute|unban|unwarn):(\d+)$/', $data, $pm)) {
        ...
        $db = getDB();
        $pRow = $db->querySingle("SELECT name, username FROM users WHERE user_id = $pTargetId AND chat_id = $chatId", true);
        ...
        if ($pAction === 'unmute') {
            ...
            $current = (int)$db->querySingle("SELECT count FROM warns WHERE user_id = $pTargetId AND chat_id = $chatId");
            if ($current <= 0) {
    ...
    $db   = getDB();
    $row  = $db->querySingle("SELECT name, username FROM users WHERE user_id = $targetId AND chat_id = $chatId", true);
    ...
    if ($action === 'mute_time') {
        ...
        if ($value === 0) {
            ...
            if ($result['ok'] ?? false) {
                ...
                sendMessage($chatId, "❌ Не удалось заблокировать. Проверьте права бота.");
            }
        } else {
...
function cmdBan(int $chatId, int $replyTo, int $adminId, ?array $target, array $args): void {
    ...
    if ($result['ok'] ?? false) {
        ...
        sendReply($chatId, $replyTo, "❌ Не удалось заблокировать. Проверьте права бота.");
    }
}
...
function cmdUnban(int $chatId, int $replyTo, int $adminId, ?array $target): void {
    ...
    if ($result['ok'] ?? false) {
        ...
        sendReply($chatId, $replyTo, "❌ Не удалось снять блокировку. Проверьте права бота.");
    }
}
...
function cmdKick(int $chatId, int $replyTo, int $adminId, ?array $target): void {
    ...
    if ($result['ok'] ?? false) {
        ...
        sendReply($chatId, $replyTo, "❌ Не удалось кикнуть. Проверьте права бота.");
    }
}
...
function cmdTempban(int $chatId, int $replyTo, int $adminId, ?array $target, array $args): void {
    ...
    if ($result['ok'] ?? false) {
        ...
        sendReply($chatId, $replyTo, "❌ Не удалось выдать временный бан. Проверьте права бота.");
    }
}
...
function cmdWarn(int $chatId, int $replyTo, int $adminId, ?array $target, array $args): void {
    ...
    $current = (int)$db->querySingle("SELECT count FROM warns WHERE user_id = $targetId AND chat_id = $chatId");
    $count   = $current + 1;

    $db->exec("
        INSERT INTO warns (user_id, chat_id, count) VALUES ($targetId, $chatId, $count)
    ...
    if ($count >= 3) {
        ...
        $now = time();
        $rsn = $db->escapeString("3 предупреждения: $reason");
        $db->exec("
            INSERT INTO bans (user_id, chat_id, banned_by, reason, banned_at)
            VALUES ($targetId, $chatId, $adminId, '$rsn', $now)
            ON CONFLICT(user_id, chat_id) DO UPDATE SET banned_by = $adminId, reason = '$rsn', banned_at = $now
...
function cmdInfo(int $chatId, int $replyTo, ?array $target): void {
    ...
    $db    = getDB();
    $warns = (int)$db->querySingle("SELECT count FROM warns WHERE user_id = $targetId AND chat_id = $chatId");

    // Мут
    $muteRow  = $db->querySingle("SELECT until FROM mutes WHERE user_id = $targetId AND chat_id = $chatId", true);
    ...
    // Бан с причиной
    $banRow   = $db->querySingle("SELECT reason, banned_at, banned_by FROM bans WHERE user_id = $targetId AND chat_id = $chatId", true);
    $banStr   = '✅ Нет';
    if ($banRow) {
        ...
        $bannerRow = $db->querySingle("SELECT name FROM users WHERE user_id = {$banRow['banned_by']} AND chat_id = $chatId", true);
        if ($bannerRow) {
    ...
    $userRow      = $db->querySingle("SELECT message_count, joined_at FROM users WHERE user_id = $targetId AND chat_id = $chatId", true);
    ...
    $reportsCount = (int)$db->querySingle("SELECT COUNT(*) FROM reports WHERE reported_id = $targetId AND chat_id = $chatId");
...
function cmdMe(int $chatId, int $replyTo, int $userId, array $fromUser): void {
    ...
    $db    = getDB();
    $warns = (int)$db->querySingle("SELECT count FROM warns WHERE user_id = $userId AND chat_id = $chatId");

    // Мут
    $muteRow = $db->querySingle("SELECT until FROM mutes WHERE user_id = $userId AND chat_id = $chatId", true);
    $isMuted = $muteRow && ($muteRow['until'] === 0 || (int)$muteRow['until'] > time());
    $muteStr = '🔊 Нет';
    if ($isMuted) {
        $until   = (int)$muteRow['until'];
    ...
    // Бан
    $isBanned = (bool)$db->querySingle("SELECT 1 FROM bans WHERE user_id = $userId AND chat_id = $chatId");
    $banStr   = $isBanned ? '🔴 Да' : '✅ Нет';

    // Репутация
    $rep      = (int)$db->querySingle("SELECT rep FROM reputation WHERE user_id = $userId AND chat_id = $chatId");

    // Статистика сообщений
    $userRow  = $db->querySingle("SELECT message_count, joined_at FROM users WHERE user_id = $userId AND chat_id = $chatId", true);
    $msgCount = $userRow ? (int)$userRow['message_count'] : 0;
    $joinedAt = $userRow ? (int)$userRow['joined_at']     : 0;
    $joinedStr = $joinedAt > 0 ? date('d.m.Y', $joinedAt) : 'Неизвестно';
...
function cmdPromote(int $chatId, int $replyTo, int $adminId, ?array $target): void {
    ...
    if (!($result['ok'] ?? false)) {
        sendReply($chatId, $replyTo, "❌ Не удалось выдать права администратора. Проверьте права бота.");
        return;
    }
...
function cmdRules(int $chatId, int $replyTo): void {
    $db      = getDB();
    $existing = $db->querySingle("SELECT text FROM rules WHERE chat_id = $chatId LIMIT 1");
...
function getRandomChatUser(int $chatId): ?array {
    $db  = getDB();
    $row = $db->querySingle(
        "SELECT user_id, name, username FROM users WHERE chat_id = $chatId ORDER BY RANDOM() LIMIT 1",
        true
    );
    return $row ?: null;
}
...
function cmdWho(int $chatId, int $replyTo, string $subject): void {
    ...
    // Берём двух случайных разных участников
    $rows = [];
    $res  = $db->query(
        "SELECT user_id, name, username FROM users WHERE chat_id = $chatId ORDER BY RANDOM() LIMIT 2"
    );
    if ($res) {
        while ($row = $res->fetchArray(SQLITE3_ASSOC)) {
            $rows[] = $row;
    ...
    if (count($rows) >= 2) {
        // Два участника — Мистер и Мистерс
        $mister  = $makeLink($rows[0]);
        $misters = $makeLink($rows[1]);
        sendReply($chatId, $replyTo, "{$emoji} Мистер <b>{$mister}</b> и Мистерс <b>{$misters}</b> {$subject}!");
    } else {
        // Один участник — просто отвечаем
        $link = $makeLink($rows[0]);
...
function getAutomodSettings(int $chatId): array {
    $db  = getDB();
    $row = $db->querySingle("SELECT * FROM automod WHERE chat_id = $chatId", true);
    if (!$row) {
...
function checkAutomod(array $msg): bool {
    ...
    if (!$reason && $text !== '') {
        ...
        if ($words) {
            while ($row = $words->fetchArray(SQLITE3_ASSOC)) {
    ...
    if (!$reason && $settings['block_flood']) {
        ...
        $row = $db->querySingle(
            "SELECT count, window FROM flood_tracker WHERE user_id = $userId AND chat_id = $chatId",
            true
        );

        if (!$row || ($nowTime - (int)$row['window']) > $window) {
            $db->exec("
                INSERT INTO flood_tracker (user_id, chat_id, count, window)
    ...
    if (!$reason && $settings['block_media']) {
        ...
        if ($isMedia) {
            ...
            $mRow = $db->querySingle(
                "SELECT count, window FROM media_tracker WHERE user_id = $userId AND chat_id = $chatId",
                true
            );

            if (!$mRow || ($nowTime - (int)$mRow['window']) > $mediaWindow) {
                $db->exec("
                    INSERT INTO media_tracker (user_id, chat_id, count, window)
                    VALUES ($userId, $chatId, 1, $nowTime)
                    ON CONFLICT(user_id, chat_id) DO UPDATE SET count = 1, window = $nowTime
                ");
            } else {
    ...
    $db->exec("
        INSERT INTO mutes (user_id, chat_id, until) VALUES ($userId, $chatId, $until)
        ON CONFLICT(user_id, chat_id) DO UPDATE SET until = $until
    ");
    apiRequest('restrictChatMember', [
        'chat_id'     => $chatId,
    ...
    $userRow  = $db->querySingle("SELECT name, username FROM users WHERE user_id = $userId AND chat_id = $chatId", true);
    $nameLink = $userRow
...
function cmdAddWord(int $chatId, int $replyTo, int $adminId, array $args): void {
    ...
    $db  = getDB();
    $esc = $db->escapeString($word);
    $exists = $db->querySingle("SELECT id FROM automod_words WHERE chat_id = $chatId AND word = '$esc'");
    if ($exists) {
...
function cmdAmnesty(int $chatId, int $replyTo, int $adminId): void {
    ...
    if ($tempRes) {
        while ($row = $tempRes->fetchArray(SQLITE3_ASSOC)) {
            $candidates[(int)$row['user_id']] = true;
        }
    }
...
function cmdBanlist(int $chatId, int $replyTo): void {
    ...
    $banInfo = [];
    if ($banRes) {
        while ($row = $banRes->fetchArray(SQLITE3_ASSOC)) {
    ...
    foreach (array_keys($candidates) as $uid) {
        ...
        $userRow  = $db->querySingle("SELECT name, username FROM users WHERE user_id = $uid AND chat_id = $chatId", true);
        ...
        if ($info) {
            ...
            $byRow    = $db->querySingle("SELECT name, username FROM users WHERE user_id = $byId AND chat_id = $chatId", true);
...
function isMuted(int $chatId, int $userId): bool {
    $db  = getDB();
    $row = $db->querySingle(
        "SELECT until FROM mutes WHERE user_id = $userId AND chat_id = $chatId",
        true
    );
...
function getTarget(array $msg, array $args, int $chatId): ?array {
    ...
    if (str_starts_with($arg, '@')) {
        ...
        $db       = getDB();
        $row      = $db->querySingle(
            "SELECT user_id, name FROM users WHERE username = '$username' AND chat_id = $chatId",
            true
        );
        if ($row) {
            return [
    ...
    if (is_numeric($arg)) {
        ...
        $db     = getDB();
        $row    = $db->querySingle(
            "SELECT name, username FROM users WHERE user_id = $userId AND chat_id = $chatId",
            true
        );
        return [
            'id'       => $userId,
...
function cmdReport(array $msg, int $chatId, int $messageId, int $userId, string $inlineReason): void {
    ...
    // Формируем имена
    $reporterRow  = $db->querySingle("SELECT name, username FROM users WHERE user_id = $userId AND chat_id = $chatId", true);
    $reportedRow  = $db->querySingle("SELECT name, username FROM users WHERE user_id = $reportedId AND chat_id = $chatId", true);
    ...
    $settings   = $db->querySingle("SELECT target_chat FROM report_settings WHERE chat_id = $chatId", true);
...
function cmdReportFromChat(int $chatId, int $messageId, int $userId, array $args): void {
    ...
    $codeEsc = $db->escapeString($code);
    // Сохраняем как внешний ключ: тот чат шлёт репорты в этот
    $db->exec("
        INSERT INTO report_settings (chat_id, target_chat) VALUES (0, $chatId)
...
function cmdReportHereSetup(int $chatId, int $messageId, int $userId): void {
    ...
    $db = getDB();
    $db->exec("
        INSERT INTO report_settings (chat_id, target_chat) VALUES ($chatId, $chatId)
        ON CONFLICT(chat_id) DO UPDATE SET target_chat = $chatId
    ");
...
function handleReportCallback(string $callbackId, int $chatId, int $messageId, int $adminId, string $data): void {
    ...
    if (preg_match('/^report_reviewed:(\d+)$/', $data, $m)) {
        ...
        $report = $db->querySingle("SELECT * FROM reports WHERE id = $reportId", true);
        if (!$report) {
        ...
        $adminRow      = $db->querySingle("SELECT name, username FROM users WHERE user_id = $adminId AND chat_id = $chatId", true);
        ...
        $reportedRow   = $db->querySingle("SELECT name, username FROM users WHERE user_id = $reportedId", true);
        $reporterRow   = $db->querySingle("SELECT name, username FROM users WHERE user_id = $reporterRow", true);
        $reportedName  = htmlspecialchars($reportedRow['name'] ?? "ID {$reportedId}", ENT_XML1);
        $reporterRow2  = $db->querySingle("SELECT name FROM users WHERE user_id = $reporterRow", true);
        $reporterName  = '';
        $reporterRow3  = $db->querySingle("SELECT name FROM users WHERE user_id = $reporterId", true);
    ...
    if (preg_match('/^report_ban_do:(\d+):(\d+):(\d+)$/', $data, $m)) {
        ...
        if ($result['ok'] ?? false) {
            ...
            apiRequest('answerCallbackQuery', ['callback_query_id' => $callbackId, 'text' => '❌ Не удалось заблокировать', 'show_alert' => true]);
        }
        return;
    }
...
function cmdCloseChat(int $chatId, int $replyTo, int $adminId): void {
    ...
    if ($result['ok'] ?? false) {
        ...
        sendReply($chatId, $replyTo, "❌ Не удалось закрыть чат. Проверьте права бота.");
    }
}
...
function cmdOpenChat(int $chatId, int $replyTo, int $adminId): void {
    ...
    if ($result['ok'] ?? false) {
        ...
        sendReply($chatId, $replyTo, "❌ Не удалось открыть чат. Проверьте права бота.");
    }
}
...
function cmdSetWelcome(int $chatId, int $messageId, int $userId, array $args, ?array $msg): void {
    ...
    $db = getDB();
    $escaped = $db->escapeString($template);
    $db->exec("
        INSERT INTO welcome_settings (chat_id, enabled, template) VALUES ($chatId, 1, '$escaped')
...
function cmdWelcomeOff(int $chatId, int $messageId, int $userId): void {
    ...
    $db = getDB();
    $db->exec("
        INSERT INTO welcome_settings (chat_id, enabled, template) VALUES ($chatId, 0, '')
...
function cmdWelcomeOn(int $chatId, int $messageId, int $userId): void {
    ...
    $db = getDB();
    $db->exec("
        INSERT INTO welcome_settings (chat_id, enabled, template) VALUES ($chatId, 1, '')
...
function cmdRep(int $chatId, int $replyTo, int $userId, ?array $target, int $value): void {
    ...
    // Обновляем репутацию
    $db->exec("
        INSERT INTO reputation (user_id, chat_id, rep) VALUES ($targetId, $chatId, $value)
    ...
    $newRep   = (int)$db->querySingle("SELECT rep FROM reputation WHERE user_id = $targetId AND chat_id = $chatId");
    $nameLink = formatUserName($target);
    $voterRow = $db->querySingle("SELECT name, username FROM users WHERE user_id = $userId AND chat_id = $chatId", true);
...
function writeModLog(int $chatId, int $adminId, int $targetId, string $action, string $reason = '', string $extra = ''): void {
    $db  = getDB();
    $now = time();
    $a   = $db->escapeString($action);
    $r   = $db->escapeString($reason);
    $e   = $db->escapeString($extra);
    ...
    $setting   = $db->querySingle("SELECT log_chat_id FROM modlog_settings WHERE chat_id = $chatId", true);
    $logChatId = (int)($setting['log_chat_id'] ?? 0);
    if ($logChatId === 0) return;

    // Данные участников
    $adminRow  = $db->querySingle("SELECT name, username FROM users WHERE user_id = $adminId  AND chat_id = $chatId", true);
    $targetRow = $db->querySingle("SELECT name, username FROM users WHERE user_id = $targetId AND chat_id = $chatId", true);
    ...
    $text  = "📋 <b>История наказаний</b> | <b>{$chatTitle}</b>\n";
    $text .= "━━━━━━━━━━━━━━━━━━━━\n";
    $text .= "🎯 Действие: {$label}\n";
...
function cmdSetLogChannel(int $chatId, int $messageId, int $adminId, ?array $msg): void {
    ...
    $db = getDB();
    $db->exec("
        INSERT INTO modlog_settings (chat_id, log_chat_id) VALUES ($chatId, $logTarget)
...
function cmdLogs(int $chatId, int $messageId, ?array $target, ?array $msg): void {
    ...
    foreach ($rows as $row) {
        ...
        if (!isset($adminCache[$adminId2])) {
            $adminRow    = $db->querySingle("SELECT name, username FROM users WHERE user_id = {$adminId2} AND chat_id = $chatId", true);
...
function cmdRemind(int $chatId, int $messageId, int $userId, array $args, ?array $msg): void {
    ...
    if (in_array(mb_strtolower($args[0]), ['отмена', 'cancel', 'удалить', 'del'], true)) {
        ...
        $row = $db->querySingle(
            "SELECT id FROM reminders WHERE id = $remId AND chat_id = $chatId AND user_id = $userId AND done = 0",
            true
        );
        deleteMessage($chatId, $messageId);
        if (!$row) {
    ...
    $fireAt  = time() + $seconds;
    $escaped = $db->escapeString(mb_substr($noteText, 0, 2000));
    $db->exec("
...
function getAntiRaidSettings(int $chatId): array {
    $db  = getDB();
    $row = $db->querySingle("SELECT * FROM antiraid WHERE chat_id = $chatId", true);
    if (!$row) {
